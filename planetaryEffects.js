...
class PlanetaryEffects {
    ...
    initEffects() {
      this.effects = {
        ...
        // Io (nivel 7) - Erupciones volcánicas y lava
        6: {
            fogParticles: [],
            distortion: {
              time: 0,
              intensity: 0.8 // Más intenso que Nexus
            },
            
            init: function() {
              this.fogParticles = [];
              
              // Partículas de vapor más densas
              for (let i = 0; i < 80; i++) {
                this.fogParticles.push({
                  x: Math.random() * this.game.width,
                  y: this.game.height - Math.random() * 100, // Salen del suelo
            ...
            update: function() {
              ...
              this.fogParticles.forEach(p => {
                ...
                // Reutilizar partículas que salen de pantalla
                if (p.y < -20) {
            ...
            draw: function(ctx) {
              ...
              for (let y = 0; y < this.game.height; y += 15) {
                const offsetX = Math.sin(y * 0.03 + this.distortion.time) * waveIntensity * 15;
                ctx.drawImage(
                  this.game.canvas,
                  0, y, this.game.width, 15,
                  offsetX, y, this.game.width, 15
                );
              }
