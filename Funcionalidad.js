...
class LunarLander {
    ...
    generateTerrain() {
      ...
      // Generar puntos de control para el terreno con Perlin Noise simulado
      const noisePoints = [];
      for (let i = 0; i <= segments; i++) {
        // Valores más altos = terreno más irregular
        const roughness = 0.8;
        noisePoints.push(Math.random() * roughness);
      }
      ...
      // Seleccionar segmentos para plataformas de aterrizaje
      for (let i = 0; i < numLandingPads; i++) {
        let segmentIndex;
        do {
          // Evitar los bordes
          segmentIndex = Math.floor(Math.random() * (segments - 4)) + 2;
        } while (landingPadSegments.includes(segmentIndex));
        
        landingPadSegments.push(segmentIndex);
      }
      ...
      for (let i = 0; i <= segments; i++) {
        ...
        let y;
        if (isLandingPad) {
          // Las plataformas de aterrizaje son planas
          ...
          // Terreno variable
          y = baseHeight - (smoothedPoints[i] * 150);
        }
    ...
    update(deltaTime) {
      ...
      // Componentes de velocidad basados en rotación
      const thrustX = Math.sin(this.lander.rotation) * this.lander.thrust * this.scaleFactor;
